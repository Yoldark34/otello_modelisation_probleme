...
public class Plateau {
	    ...
	    public byte getJeton(int x, int y)
	    {	// retourne la couleur du jeton présent sur le plateau aux coordonnées x,y
	    	return othellier[x][y];
	    }

	    public int nombreJetons(int couleur)
	    {	// retourne le nombre de jetons, de la couleur donnée, présents sur le plateau
	        int nb = 0;
	        synchronized (othellier)
	        {for(int i = 0; i < NUM_LIGNES; i++)
	            {for(int j = 0; j < NUM_LIGNES; j++)
